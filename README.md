...
或者用 git：
```bash
git clone https://github.com/你的用户名/advisor-finder.git
cd advisor-finder
```
...
将简历、个人陈述等文件（`.txt` / `.docx` / `.pdf` 格式）放入 `个人情况/` 文件夹，Agent 会据此给出更精准的匹配。
...
- 工具包括：DuckDuckGo 搜索、网页抓取（requests + BeautifulSoup）、本地文件读写
- 多轮对话，带完整上下文记忆
...
- `.env` 文件包含你的 API Key，**不要分享给他人**
- 网页抓取结果依赖目标网站的可访问性，部分高校网站可能需要校内网
- 建议使用前在 `个人情况/` 放入个人资料，推荐质量会明显提升
